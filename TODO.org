...
** TOOO cannot select (data) item to be renamed from the module exports list
...
** DONE Does not properly load a cabal exe Main module if not called Main.hs e.g. MainHare.hs
