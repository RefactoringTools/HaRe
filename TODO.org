...
** TODO rename mod to parsed complains of existing variable named parsed.
   ...
   [[file:~/mysrc/github/alanz/HaRe/test/UtilsSpec.hs::let%20mod%20%3D%20GHC.pm_parsed_source%20$%20GHC.tm_parsed_module%20t][file:~/mysrc/github/alanz/HaRe/test/UtilsSpec.hs::let mod = GHC.pm_parsed_source $ GHC.tm_parsed_module t]]
     - Also, get "error attempting to use module `main:TestUtils' (/home/alanz/mysrc/github/alanz/HaRe/test/TestUtils.hs) which is not loaded" 
** TODO The elisp test for buffers to save includes directories that have been modified
** DONE check cabal support
     - [x] It does not seem to pick up the main files. Need to see how to
         set those.
     - [x] pull request for ghc-mod to return those
     - [x] should possibly make a setting to include targets from
...
** DONE make sure clientModsAndFiles and serverModsAndFiles us rsGraph
   - clientModsAndFiles does
   - serverModsAndFiles does not require it
...
** DONE After renaming, block comments with {- -} start one away from the margin
...
** DONE layout problem after renaming pats1 (1469,38) of MoveDef.hs
