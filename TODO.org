...
** DONE renaming with length change does not adjust surrounding layout for do/let/where/of etc
    - 'of' complete
    - 'do' complete
    - 'let' complete
    - 'where' complete
    - nested layout complete
** DONE rename tid to tidlong in MissingH-1.2.0.2/src/Data/Progress/Meter.hs (164,16) does not fix layout after the do
...
** DONE adjustLayoutAfterRename might be usable once only when realigning for the final tokens.
