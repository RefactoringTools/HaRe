...
** TODO make sure clientModsAndFiles and serverModsAndFiles us rsGraph
...
** TODO After renaming, block comments with {- -} start one away from the margin
   e.g. syz-0.2.0.0/Data/Generics/Zipper.hs rename reduceAncestors to
   reduceAncestors1 on line 354, the comment starting on line 362
   shifts
** TODO complete the undefined's in Layout.hs
** TODO after accepting a refactoring, reload the buffer into emacs
** TODO layout problem after renaming pats1 (1469,38) of MoveDef.hs
   The second let in the subordinate do clause is shifted right by one
...
** TODO renaming in MissingH-1.2.0.2/src/Control/Concurrent/Thread/Utils.hs
   - rename (34,0) to runInThread1, gives
     "Cannot add module Control.Concurrent.Thread.Utils to context: not a home module"
     See http://parenz.wordpress.com/2013/08/17/ghc-api-interpreted-compiled-and-package-modules/ for pointers
** TODO renaming in HaRe/src/Language/Haskell/Refact/Utils.hs
   - (486,6) to clientModsAndFiles1 gives
     "attempting to use module `main:Language.Haskell.Refact.Utils.DualTree'
       (/home/alanz/mysrc/github/alanz/HaRe/./src/Language/Haskell/Refact/Utils/DualTree.hs) which is not loaded"
** TODO cabalAllTargets seems to no return exe and test for HaRe.cabal
   - but only if not operating from the actual directory holding the
     cabal file. Hmm.
** TODO initGhcSession must lump all the library files into a single target.
