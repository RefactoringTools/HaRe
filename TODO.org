* HaRe todo
** DONE ghc-hare fails on GhcVersionSpecific, Could it be the LANGUAGE CPP?
...
** DONE Strategy for CPP code: how to preserve the preprocessor directives?
    - Take original source, extract prepocessor directives (lines
      starting #), turn them into line comments and merge in.
    - Problem is, in an #if/#then/#else the part not included gets
      thrown away.
    - Cannot simply tokenise it, the refactorings will go crazy. Have
      to some-how detect them and convert them into blockcomments, to
      carry through.
